...
import { TIER_LABELS, computeRationaleScore } from "@/lib/flowScoring";
import type { RubricCategory, Tier } from "@/lib/content/rubric";
...
/**
 * Result of a quest's flow canvas: the tier and feedback its rubric produced,
 * the score breakdown against the rubric's category maxes, and — when the
 * question asks for one — the written reason that adds the Rationale points.
 */
export function FlowResultClient({
  submissionId,
  questOrder,
  timeExpired,
  elapsedSeconds,
  ...
  tier: Tier;
  message: string;
  reflectionConfig: { prompt: string; placeholder?: string; required: boolean } | null;
  existingReflection: string;
  nextHref: string;
  nextLabel: string;
}) {
  const withReflection = reflectionConfig !== null;
    ...
    <div className="mx-auto max-w-[600px] px-5 pt-[50px] pb-24 text-center">
      ...
      {withReflection && (
        <div className="text-left">
          <label className="mb-2 block text-[13.5px] font-semibold">{reflectionConfig?.prompt}</label>
          <Textarea
            value={reflection}
            disabled={saved}
      ...
      <Button
        ...
        disabled={Boolean(reflectionConfig?.required) && !saved}
        onClick={() => {
