...
export interface QuizTransport {
  answer(questionId: string, answer: unknown): Promise<{ ok: true; reveal: Reveal | null } | { ok: false; error: string }>;
  finish(): Promise<void>;
  timeUp(): void;
}
...
export function QuizPlayer({
  ...
  const [page, setPage] = useState(() => {
    const firstOpen = initial.findIndex((q) => (checkMode === "instant" ? !q.reveal : q.answer === null));
  ...
  const setAnswer = (answer: unknown) => setItems((prev) => prev.map((q, i) => (i === page ? { ...q, answer } : q)));
  ...
  async function save(): Promise<boolean> {
    if (transport) {
      ...
      setItems((prev) => prev.map((q, i) => (i === page ? { ...q, answer: value, reveal: r.reveal } : q)));
      return true;
    }
    const res = await post("/api/quest/answer", { order, questionId: current.question.id, answer: value });
    ...
    setItems((prev) => prev.map((q, i) => (i === page ? { ...q, answer: value, reveal: res.data.reveal ?? null } : q)));
    return true;
  }
  ...
  const primary = (() => {
    if (checkMode === "instant") {
      if (!current.reveal) {
        return { label: "Konfirmasi Jawaban", variant: "primary" as const, disabled: !answered, onClick: () => run(async () => void (await save())) };
      }
      if (!isLastPage) return { label: "Soal Berikutnya →", variant: "primary" as const, disabled: false, onClick: () => setPage(page + 1) };
      return { label: hasFlowAfter ? "Lanjut Susun Flow →" : finishLabel, variant: "gold" as const, disabled: false, onClick: () => run(finish) };
    }
    if (!isLastPage) {
      return {
        label: "Berikutnya →",
    ...
    <Guard tracked={!transport} questOrder={order}>
      <div className="mx-auto max-w-[600px] px-5 pt-[50px] pb-20">
        <div className="flex items-start justify-between gap-3">
          ...
          {secondsLeft !== null && (
            <span className={`rounded-[20px] border px-3 py-1 text-[12.5px] font-semibold tabular-nums ${secondsLeft <= 30 ? "border-danger text-danger" : "border-border-light text-muted"}`}>
        ...
        {!single && (
          <div className="mb-4">
            ...
            <div className="h-1.5 overflow-hidden rounded bg-surface2">
              <div className="h-full rounded bg-teal transition-all" style={{ width: `${((page + 1) / items.length) * 100}%` }} />
            </div>
          </div>
        )}
        ...
        {current.question.help && <p className="-mt-1 mb-3 text-[13.5px] text-muted">{current.question.help}</p>}
        {single && intro && <IntroCallout intro={intro} />}
        <MediaList media={current.question.media} className="mb-4" />

        <div className="mb-6 mt-5">
          <QuestionInput key={current.question.id} question={current.question} value={value} onChange={setAnswer} reveal={reveal} disabled={busy} />
        </div>
        ...
        {reveal && reveal.feedback.length > 0 && (
          <div className={`mb-5 rounded-[10px] border px-4 py-3.5 text-[14px] leading-[1.6] ${fullyRight ? "border-success bg-[rgba(123,201,126,0.1)]" : "border-gold bg-[rgba(240,172,63,0.1)]"}`}>
            {reveal.feedback.map((f, i) => (
              <RichText key={i} source={f} paragraphClassName="mb-1.5 last:mb-0" listClassName="mb-1.5 list-disc pl-5" />
            ))}
          </div>
        )}
        {reveal && reveal.feedback.length === 0 && !single && (
          <div className={`mb-5 text-[13.5px] font-semibold ${fullyRight ? "text-success" : "text-gold"}`}>
            {fullyRight ? "✓ Benar!" : reveal.score.total <= 1 ? "✗ Belum tepat" : `${reveal.score.correct} dari ${reveal.score.total} benar`}
          </div>
        )}

        <div className="flex flex-wrap items-center gap-3">
          {checkMode === "end" && page > 0 && (
            <Button variant="ghost" disabled={busy} onClick={() => setPage(page - 1)}>
              ← Sebelumnya
            </Button>
          )}
          <Button variant={primary.variant} disabled={primary.disabled || busy} onClick={primary.onClick}>
            {busy ? "Menyimpan…" : primary.label}
          </Button>
        </div>
        {error && <div className="mt-3 rounded-[9px] border border-danger bg-[rgba(242,112,92,0.1)] px-3.5 py-2.5 text-[13.5px] text-danger">⚠️ {error}</div>}
      </div>
    </Guard>
  );
}
...
function IntroCallout({ intro }: { intro: string }) {
  return (
    <div className="mb-6 rounded-r-[10px] border-l-[3px] border-teal bg-surface2 px-[18px] py-4 text-[14.5px] leading-[1.65]">
      <b className="text-teal">Skenario:</b> <RichText source={intro} className="inline" paragraphClassName="inline" />
    </div>
  );
}

/** A participant's play counts tab switches (see FocusGuard); the builder's doesn't. */
function Guard({ tracked, questOrder, children }: { tracked: boolean; questOrder: number; children: ReactNode }) {
  return tracked ? <FocusGuard questOrder={questOrder}>{children}</FocusGuard> : <>{children}</>;
}
